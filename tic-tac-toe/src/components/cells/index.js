...
import { updateActionCell, updateTurnUser } from '../../stores/slice.js'
...
const Cell = ({ row, column }) => {
  ...
  const renderStrikeComponentCell = (cell = {}) => {
    const imageSrc =
      cell?.action === ACTION_BUTTON.Cross ? IMAGE.Cross : IMAGE.Nought;
    let elementStrike = [];
      ...
      Object.keys(cell).forEach(function (key) {
        if (cell[key] === true) {
          const combinedClasses =
            renderTypeClass[key] + " " + renderTypeClass[cell.action];
